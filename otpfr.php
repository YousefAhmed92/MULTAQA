...
if (isset($_POST['resend'])) {
    ...
    $expiration_timeotpfr->add(new DateInterval('PT60S')); // Set expiration time to 60 seconds
...
if(isset($_POST['submit'])){
    ...
    $otp = $_POST['digit1'] . $_POST['digit2'] . $_POST['digit3'] . $_POST['digit_4'];
     $current_timeotpfr = new DateTime(); // Update current time each time the form is submitted
     $expiration_timeotpfr = new DateTime($_SESSION['expiration_timeotpfr']);

     if ($current_timeotpfr > $expiration_timeotpfr) {
        $error = "Expired OTP. Please press 'resend'.";
        unset($_SESSION['otp']);

    }elseif($_SESSION['otp'] == $otp){
        echo "Validation completed";
        header("location:resetpassfree.php");
        unset($_SESSION['otp']);
        exit();
...
<body>
    <div class="container"> 
        ...
        <form method="POST" action="otpfr.php">
